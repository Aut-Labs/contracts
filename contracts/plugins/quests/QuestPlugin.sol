...
contract QuestPlugin is QuestsModule, SimplePlugin {
    ...
    function createTask(uint256 questId, uint256 tasksPluginId, string memory uri)
        ...
        onlyPending(questId)
    {
        IPluginRegistry.PluginInstance memory pluginInstance = pluginRegistry.getPluginInstanceByTokenId(tasksPluginId);
    ...
    function _addTask(uint256 questId, PluginTasks memory task) private {
        require(idCounter.current() >= questId, "invalid quest id");
        IPluginRegistry.PluginInstance memory plugin = pluginRegistry.getPluginInstanceByTokenId(task.pluginId);

        require(plugin.pluginAddress != address(0), "Invalid plugin");
        bool isInstalled = pluginRegistry.pluginDefinitionsInstalledByDAO(daoAddress(), plugin.pluginDefinitionId);
